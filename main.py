...
if not (0 < RADIUS <= 250): raise ValueError("Longitude must be between 0 and 250 NM")
...
def check_aircraft() -> bool:
    ...
    try:
        ...
        for ac in data.get("ac", []):
            ...
            lat = ac.get("lat")
            lon = ac.get("lon")
            track = None if (ac.get("track") is None) else int(round(ac.get("track"))) # degrees
            dist = get_distance(LAT, LON, lat, lon) # Distance of aircraft from surveillance point in NM
            azimuth = int(round(get_azimuth(LAT, LON, lat, lon))) # Azimuth of aircraft from surveillance point in degrees
...
if __name__ == "__main__":
    ...
    try:
        with open(CSV_FILE, "x", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["timestamp", "callsign", "regis", "hex", "type", "desc", "alt", "vspeed", "lat", "lon", "track", "dist", "azimuth"])
    ...
    print(f"📡 Monitoring airspace within {RADIUS} NM from https://www.openstreetmap.org/#map=9/{LAT}/{LON} between {MIN_ALT} and {MAX_ALT} ft")
